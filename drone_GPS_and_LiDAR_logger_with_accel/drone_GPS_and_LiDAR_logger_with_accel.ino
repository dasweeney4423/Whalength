// Program to talk to Neo-6M GPS and lightware SF-11 laser rangefinder and log to SparkFun SDcard board.
// Code was moriginally from EvaLeunissen/Whalength github repo but was then modified into the jordkl/Whalength repo before finally being modified to suit METR needs by David Sweeney on 16 Sept 2021
...
struct imu_data get_IMU_readings() {
  ...
  // next two lines may need to be modified depending on IMU orientation relative to lidar unit
  horiz_mag = sqrt(results.Grav_x * results.Grav_x + results.Grav_z * results.Grav_z);
  results.tilt_deg = abs(atan(horiz_mag / results.Grav_y) * 180. / M_PI);
